class FakeDevice:
    def __init__(self):
        self._output_source_priority = 1
    ...
    async def query(self, cmd: str) -> bytes:
        stubs = {
            ...
            "QPIRI" : "(230.0 21.7 230.0 50.0 21.7 5000 5000 48.0 51.0 50.8 58.4 54.8 2 030 090 0 <osp> 1 9 01 0 0 52.0 0 1 000xx\r",
        ...
        raw = stubs[cmd]
        if cmd == "QPIRI":
            raw = raw.replace("<osp>", f"{self._output_source_priority}")

        return raw.encode()
    
    async def set_output_source_priority(self, priority : int) -> bool:
        if priority in [0,1,2]:
            self._output_source_priority = priority
            return True
